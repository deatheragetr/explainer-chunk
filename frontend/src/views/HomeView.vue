...
// Import DirectoryBreadcrumb normally
import DirectoryBreadcrumb from '@/components/DirectoryBreadcrumb.vue'
// Import DocumentUploadModal using defineAsyncComponent to avoid initialization issues
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(async () => {
      console.log('HomeView mounted, route:', route.fullPath)
      loading.value = true

      try {
        // Wait for directory store initialization if needed
        if (directoryStore.directories.length === 0) {
          await directoryStore.fetchAllDirectories()
        }

        // Check what route we're on and explicitly handle it
        if (route.name === 'home') {
          console.log('On home route - explicitly setting root directory')
          // For root route, forcibly clear any existing selection
          directoryStore.currentDirectory = null
          directoryStore.breadcrumbs = [{ _id: null, name: 'Home', path: '/' }]

          // Fetch root contents
          await directoryStore.fetchDirectoryContents(null)
        } else if (route.name === 'directory' && route.params.id) {
          console.log('On directory route with ID:', route.params.id)
          const directoryId = route.params.id as string

          // Fetch directory contents for this specific ID
          await directoryStore.fetchDirectoryContents(directoryId)
        }

        // Now fetch documents for whatever directory is current
        nextCursor.value = null
    ...
    watch(
      ...
      (newDirectory, oldDirectory) => {
        if (initialLoadCompleted.value && newDirectory !== oldDirectory) {
          console.log('Directory changed in store:', newDirectory?.name ?? 'Root')
          nextCursor.value = null
          fetchDocuments()
        }
      },
