...
class AISummaryService:
    ...
    async def query_similar_chunks(self, query: str, document_id: str, top_k: int = 5) -> List[Dict[str, Any]]:
    ...
    async def get_document_chunks(self, document_id: str, top_k: int = 10) -> List[Dict[str, Any]]:
        ...
        results = await asyncio.to_thread(
            self.index.query,
            vector=[0] * self.model_pair_config['embedding_model']['dimension'],  # Dummy vector
            filter={"document_id": document_id},
            top_k=top_k,
            include_metadata=True
        )
    ...
    async def _make_api_call(self, messages: List[ChatCompletionMessageParam], max_tokens: int) -> str:
    ...
    async def most_advanced_summarize(self, document_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "metadata": {
                ...
                "final_summary_length": self.embedding_generator.num_tokens_from_string(final_summary),
